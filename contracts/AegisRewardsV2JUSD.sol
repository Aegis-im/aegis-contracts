...
/**
 * @title AegisRewardsV2JUSD
 * @notice Rewards contract with cumulative Merkle distribution
 *         and cross-chain support via LayerZero OFT bridging
 */
contract AegisRewardsV2JUSD is IAegisRewardsErrors, AccessControlDefaultAdminRules, ReentrancyGuard {
    ...
    /// @dev Total JUSD reserved for unclaimed Merkle rewards
    ...
    /// @dev Returns total JUSD reserved for unclaimed Merkle rewards
    ...
    function depositRewards(bytes calldata requestId, uint256 amount) external onlyRole(DEPOSITOR_ROLE) {
        ...
        if (jusd.balanceOf(address(this)) < _merklePoolBalance) revert InsufficientContractBalance();
    ...
    function sendToStaking(uint256 amount) external onlyRole(TRUSTED_SIGNER_ROLE) {
        ...
        _merklePoolBalance -= amount;
        jusd.safeTransfer(stakingContract, amount);
    ...
    function claimMerkleRewards(
        ...
        jusd.safeTransfer(_msgSender(), claimable);
    ...
    function rescueMerkleRewards(
        ...
        jusd.safeTransfer(to, claimable);
    ...
    /// @notice Bridge JUSD to a destination chain via LayerZero OFT adapter
    ...
    function _executeBridge(
        ...
        // Approve OFT adapter to spend JUSD
        jusd.forceApprove(address(oftAdapter), amount);
