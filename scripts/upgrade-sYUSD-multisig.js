...
async function main() {
  ...
  // Get current gas price and set appropriate values for BSC
  let gasOptions = {}

  if (network.chainId === 56n || network.chainId === 97n) { // BSC Mainnet or Testnet
    const feeData = await ethers.provider.getFeeData()
    console.log('Current gas price:', ethers.formatUnits(feeData.gasPrice || 0n, 'gwei'), 'gwei')

    // Use EIP-1559 style transaction for BSC
    ...
    gasOptions = {
      ...
      gasLimit: 2000000, // Standard gas limit for contract deployment
    }
    console.log('Using max fee per gas:', ethers.formatUnits(gasOptions.maxFeePerGas, 'gwei'), 'gwei')
    console.log('Using max priority fee per gas:', ethers.formatUnits(gasOptions.maxPriorityFeePerGas, 'gwei'), 'gwei')
    console.log('Estimated cost:', ethers.formatEther(gasOptions.maxFeePerGas * BigInt(gasOptions.gasLimit)), 'BNB')
  }

  // Use pre-deployed implementation address
  const newImplAddress = process.env.IMPLEMENTATION_ADDRESS || '0x42940C329e2A4d0bD0e828b7815C90111065415e'
  ...
  console.log('')
  console.log('2. Test the new paused functionality:')
  console.log('   # Check current paused state')
  console.log(`   npx hardhat run --network ${network.name} -e "const c = await ethers.getContractAt('sYUSD', '${proxyAddress}'); console.log('Paused:', await c.paused())"`)
