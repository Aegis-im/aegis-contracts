...
describe('#AegisOracleJUSD', () => {
  describe('#setOperator', () => {
    describe('success', () => {
      it('should add new operator', async () => {
        const [owner, operator] = await ethers.getSigners()

        const contract = await ethers.deployContract('AegisOracleJUSD', [[owner], owner])
  ...
  describe('#updateJUSDPrice', () => {
    describe('success', () => {
      it('should update jusd price', async () => {
        ...
        await expect(contract.updateJUSDPrice(price)).to.
          emit(contract, 'UpdateJUSDPrice').
          withArgs(price, timestamp)

        await expect(contract.jusdUSDPrice()).to.be.eventually.equal(price)
    ...
    describe('error', () => {
      it('should revert when caller is not an operator', async () => {
        ...
        const contract = await ethers.deployContract('AegisOracleJUSD', [[], owner])

        await expect(contract.updateJUSDPrice(99963000)).to.be.
